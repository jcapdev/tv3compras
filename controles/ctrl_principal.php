		...
		if(!empty($checando)){
			switch ($_GET["operacion"]){
				...
				case "consultarMO":
					...
					//$resultado = $ordenes->fetch_All();
					
				    //echo  $total = $ordenes->rowCount();

					//var_dump($ordenes);		

					//echo "Total". count($ordenes);
					...
					$hcont = "<h2>Consulta de mis órdenes</h2>
						...
						<table id='hTblMisOrdenes' class='zebra'>
							...
							$hcont=$hcont."<ul class ='pagination'>
											 <li><a href=''>Anterior</a></li>";

									      for ($i=0; $i <$paginas; $i++)
									      	{
									      $pagi = $i +1;		
									      $hcont=$hcont."<li><a >$pagi</a></li>
									      ";
									      	}
									       												 
							$hcont=$hcont. "<ul><li class='last' id='btnConsultarMO'><a><span>Consultar mis órdenes</span></a></li></ul>";

							$hcont=$hcont." <input type='submit' id= 'btnConsultarMO' value='Enviar orden' >";
