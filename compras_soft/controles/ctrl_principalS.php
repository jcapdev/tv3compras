			...
			if(!empty($checando)){
				switch ($_GET["operacion"]){
					case "consultarOrdenesDeptos":
						$orden = new orden();
						$ordenes = $orden->obtenerOrdenesSupervisorCompras();
						if($ordenes!=null){
					...
					case "consultarOrdenesAprobadas":
						$orden = new orden();
						$ordenes = $orden->obtenerOrdenesAprobadas();
						if($ordenes!=null){
							$hcont = "<h2>Consulta de órdenes aprobadas por gerente administrativo</h2>
								<br />
								<div id='idOrdenes'>
								<table id='hTblOrdenesAprobar' class='zebra'>
									<thead><tr><th style='width:20px;'>Folio</th><th>Nombre Trabajador</th><th>Departamento</th><th>Fecha y hora</th><th></th></tr></thead>";
